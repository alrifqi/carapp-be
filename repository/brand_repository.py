...
async def get_brands(page_size: int, page_num: int) -> List[BrandSchema]:
    brands = []
    async for brand in brands_collection.find().skip(database_util.calculate_page_to_skip(page_size, page_num)).limit(
            page_size).sort("created_at", pymongo.DESCENDING):
        brands.append(brand)
    return brands


async def create_brand(payload: BrandCreateSchema) -> BrandSchema:
    new_brand = await brands_collection.insert_one(
        {"name": payload.name, "description": payload.description, "logo": payload.logo,
...
async def update_brand(id: str, payload: BrandCreateSchema) -> BrandSchema:
    update_brand = await brands_collection.update_one({"_id": id}, {
        "$set": {"name": payload.name, "description": payload.description, "logo": payload.logo,
